...
export function UploadPictureInterceptorFactory(fieldName: string) {
  ...
  class UploadPictureInterceptor implements NestInterceptor {
    public readonly logger = new Logger(UploadPictureInterceptor.name);
      ...
      this.interceptor = new (FileInterceptor(fieldName, {
        fileFilter: (req, file, cb) => {
          ...
          if (!allowedMimeTypes.includes(file.mimetype)) {
            this.logger.warn(
              `Failed to upload file. Invalid file type: ${file.mimetype}`,
            );
            return cb(
              new BadRequestException(
                'Only jpg, jpeg, and png image types are allowed!',
              ),
              false,
            );
          }
          if (file.size > 2 * 1024 * 1024) {
            this.logger.warn(
              `Failed to upload file. File size exceeds limit: ${file.size}`,
            );
        ...
        catchError((err) => {
          ...
          if (request.file) {
            try {
              ...
              this.logger.error(
                `Failed to delete file ${request.file.path}: ${err.message}`,
              );
            }
          }
